...
public class Browser {
	...
	/**
	 * Sets the current expo. This should adhere to the format of "PAX [expo]"
	 * or just "[expo]". Using a different format may result in Browser or
	 * program inoperability. The expo set is used multiple times throughout the
	...
	/**
	 * Returns the expo currently set. This should adhere to the format of
	 * "PAX [expo]" or just "[expo]".
	 *
	...
	/**
	 * Enables the checking of the [expo].paxsite.com/registration page for the
	...
	/**
	 * Checks whether or not the program should check the PAX Registration
	 * website.
	 *
	...
	/**
	 * Checks whether or not the PAX website is updated. This checks for the
	 * Register Online button on the [expo].paxsite.com/registration page. If
	 * found, it reads the current href (hyperlink) for the button, and if it
	 * doesn't link to the [expo].paxsite.com website (when tickets are all sold
	 * out, PAX switches it to that), it returns true. This method also sets the
	 * Status window website link text. Note that this returns false if
	 * {@link #enablePaxWebsiteChecking() enablePaxWebsiteChecking()} has not
	 * been called.
	 *
	...
	/**
	 * Finds the link of the Register Now button on the PAX website. This scans
	 * EVENT.paxsite.com/registration for the Register Now button link, and
	...
	public static String getCurrentButtonLinkLine() {
		...
		try {
			...
			ErrorHandler
					.printError(
							"ERROR",
							"An unknown error has occurred while attempting to read the PAX website.",
							e);
			...
			try {
				...
				// nothing to see here
				System.out
						.println("Note: Unable to close InputStream for getCurrentButtonLinkLine()");
				ioe.printStackTrace();
			}
		}
		return "NoFind";
	...
	/**
	 * Parses the link out of the given line of HTML. Note that this requires an
	 * href with the link surrounded by quotation marks for this to work.
	 * Furthermore, the link can't have any quotation marks in it (though if it
	 * does, you have bigger problems).
	 *
	 * @param parse
	 *            The HTML to parse
	...
	public static String parseHRef(String parse) {
		...
		try {
			...
			parse = parse.substring(parse.indexOf("href=") + 6); // Get index of
																	// link
			parse = parse.substring(0, parse.indexOf("\"")); // Remove
																// everything
																// after the
																// link
	...
	/**
	 * Sets the website link used for the program. Note that this does NOT check
	 * for invalid links, and therefore should be used with caution. Invalid
	 * links will result in failure to check the proper PAX website. Ex:
	 * http://prime.paxsite.com
	 *
	 * @param link
	 *            The FULL address (http:// as well!) to check for updates.
	 */
	...
	/**
	 * Returns the HTTP address of the given PAX Expo. Be sure to only use the
	 * name of the expo (ex: prime) OR the full name (ex: pax prime) as the
	 * argument.
	 *
	 * @param expo
	 *            The PAX expo to get the website link for
	 * @return The website link of the specified expo, or the PAX Prime link if
	 *         invalid.
	 */
	public static String getWebsiteLink(String expo) {
		...
		switch (expo.toLowerCase()) { // toLowerCase to lower the possibilities
										// (and readability)
		case "prime":
		case "pax prime":
			return "http://prime.paxsite.com";
	...
	/**
	 * Tells the program to check the Showclix website when checking for ticket
	 * sales. Note that this uses the Showclix API (api.showclix.com) to check
	 * for events.
	 */
	...
	/**
	 * Checks whether or not the program is checking the Showclix website. If
	 * true, the program uses the Showclix API (api.showclix.com) to check for
	 * new events.
	 *
	...
	/**
	 * Checks whether or not the Showclix website has a new event. This uses the
	 * Showclix API to get the latest Event ID
	 * (https://www.showclix.com/Event/######) from the API. This uses the
	 * {@link #getLatestShowclixID(java.lang.String)} method to get the most
	 * recent ID.
	 *
	...
	/**
	 * Gets the latest Showclix ID for PAX events. Note that this checks PAX
	 * Prime, East, South and Aus events. If you want to check a specific
	 * event's Showclix ID, see {@link #getLatestShowclixID(java.lang.String)}.
	 *
	 * @return The most recent Showclix ID
	 */
	public static int getLatestShowclixID() {
		int maxId;
		maxId = Math.max(getLatestShowclixID("Prime"),
				getLatestShowclixID("East"));
	...
	/**
	 * Gets the latest Showclix ID for a specific PAX event. This uses the
	 * Showclix API to find all of the events listed under a specific seller. In
	 * this case, each PAX event (Prime, East, etc) has a separate Showclix
	 * seller account associated with it. This uses the fact that every new
	 * Showclix event has a larger ID than before, and simply returns the
	 * largest Event ID for the specific expo given.
	 *
	 * @param expo
	 *            The expo to check
	 * @return The most recent Showclix ID
	...
	/**
	 * Checks the link given against the most recent Showclix link. This uses
	 * the currently-set expo link. Note that if this returns false, it opens up
	 * an ErrorHandler window with a NOTE-style warning, and opens the most
	 * recent Showclix link in the browser.
	 *
	 * @param link
	 *            The link to check
	...
	public static boolean checkShowclixLink(String link) {
		...
		if (!showLink.equals(link)) {
			ErrorHandler
					.printError(
							"NOTE",
							"NOTE: The last found Showclix link has changed! New link:\n"
									+ showLink
	...
	/**
	 * Returns the link of the most recent Showclix event. Note that this gets
	 * the most recent from the current expo.
	 *
	 * @return The link of the most recent Showclix event
	 */
	public static String getShowclixLink() {
		try {
			return "https://showclix.com/event/"
					+ getLatestShowclixID(getExpo());
		} catch (Exception e) {
			ErrorHandler.printError(
					"ERORR checking the Showclix website for updates!", e);
			return null;
		}
	}

	/**
	 * Sets the Showclix ID to check against. Note that this makes the program
	 * check to see if the showclix ID found is DIFFERENT than the one it has,
	 * not if it's bigger. If the ID is -1, it will not be set.
	 *
	 * @param ID
	 *            The Showclix Event ID to set
	 */
	public static void setShowclixID(int ID) {
		if (ID == -1) {
			System.out
					.println("Unable to set most recent Showclix Event ID -- invalid ShowclixID!");
			return;
		}
	...
	/**
	 * Returns the link to the Showclix API of the given expo. The expo should
	 * be the name of the expo (Prime, East) or have PAX in front of it (PAX
	 * Prime, PAX East, etc). Any other name (PAX, PAX Invalid, etc) will return
	 * the PAX Prime link.
	 *
	 * @param expo
	 *            The name of the expo
