...
public class SettingsHandler {
	 ...
	 *            True or false
	 */
	public static void setSaveAll(boolean refreshTime, boolean showclix,
			boolean pax, boolean event, boolean email, boolean cellnum) {
	...
	/**
	 * Saves all the preferences. Note that this uses the values currently
	 * assigned to the program, so if Setup hasn't been completed yet, some
	 * Preferences will likely be incorrect.
	 */
	public static void saveAllPrefs() {
		saveAllPrefs(PAXChecker.getRefreshTime(),
				Browser.isCheckingPaxWebsite(), Browser.isCheckingShowclix(),
				Browser.getExpo());
	}

	/**
	 * Saves the Preferences given into a Preferences file. The Preferences are
	 * located in the user's registry (for Windows). Note that this also saves
	 * the cell number given. If you want to save a different cell number, see
	 * {@link #saveCellNum(String)}.
	 *
	 * @param refreshTime
	 *            The time (in seconds) between refreshes
	 * @param checkPax
	 *            True to check the PAX website, false to not
	 * @param checkShowclix
	 *            True to check the Showclix website, false to not
	 * @param expo
	 *            The Expo being checked for. Note it should be in "PAX XXXX"
	 *            format.
	 */
	public static void saveAllPrefs(int refreshTime, boolean checkPax,
			boolean checkShowclix, String expo) {
		try {
			myPrefs.sync();
		} catch (BackingStoreException bSE) {
			ErrorHandler
					.printError("Unable to sync Preferences! Preferences will not be saved.");
			bSE.printStackTrace();
			return;
		}
		try {
			...
			saveCellNum();
			saveEmail();
			System.out.println("Pax = " + checkPax + ", Showclix = "
					+ checkShowclix + ", Expo = " + expo);
			myPrefs.flush();
		} catch (BackingStoreException bSE) {
	...
	/**
	 * Sets whether or not to save the refresh time of the program. Note that
	 * you still must commit the changes using
	 * {@link #saveAllPrefs(int, boolean, boolean, boolean, java.lang.String, java.lang.String)
	 * saveAllPrefs()}.
	 *
	 * @param save
	 *            True to save refresh time, false to not
	 */
	...
	/**
	 * Sets whether or not to save the Check Showclix option. Note that you
	 * still must commit the changes using
	 * {@link #saveAllPrefs(int, boolean, boolean, boolean, java.lang.String, java.lang.String)
	 * saveAllPrefs()}.
	 *
	 * @param save
	...
	/**
	 * Sets whether or not to save the Check PAX Website option. Note that you
	 * still must commit the changes using
	 * {@link #saveAllPrefs(int, boolean, boolean, boolean, java.lang.String, java.lang.String)
	 * saveAllPrefs()}.
	 *
	...
	/**
	 * Sets whether or not to save the event selected. Note that you still must
	...
	/**
	 * Sets whether or not to save the email address used. Note that you still
	 * must commit the changes using
	 * {@link #saveAllPrefs(int, boolean, boolean, boolean, java.lang.String, java.lang.String)
	 * saveAllPrefs()}.
	 *
	 * @param save
	...
	/**
	 * Sets whether or not to save the cell number used. Note that you still
	 * must commit the changes using
	 * {@link #saveAllPrefs(int, boolean, boolean, boolean, java.lang.String, java.lang.String)
	 * saveAllPrefs()}.
	 *
	...
	/**
	 * Saves the program's email address used to the Preferences. This uses the
	 * email address currently set in Email.java.
	 *
	 ...
	 *
	 * @param email
	 *            The email address to save to the Preferences file
	 */
	 ...
	 *
	 * @param cellNum
	 *            The cell number to save to the Preferences file
	 */
	...
	/**
	 * Saves the current cell number (address) to the Preferences file. Note
	 * that this gets the email from {@link paxchecker.Email#getTextEmail()} or
	...
	private static void saveCellNum() {
		if (saveCellnum) {
			if (Email.getAddressList().isEmpty()) {
				...
				myPrefs.put(PREFTYPES.PAXCHECK_CELLNUM.name(),
						Email.convertToString(Email.getAddressList()));
			}
		} else {
			myPrefs.remove(PREFTYPES.PAXCHECK_CELLNUM.name());
		}
	 ...
	 * 
	 * @param expo
	 *            The expo for which this link applies
	 * @param link
	 *            The link to save
	 */
	public static void saveLastEvent(String expo, String link) {
		myPrefs.put(
				PREFTYPES.PAXCHECK_LAST_EVENT.name() + "_" + expo.toUpperCase(),
				link);
		try {
			myPrefs.sync();
		} catch (BackingStoreException bSE) {
	...
	/**
	 * Checks whether or not the program saved the Check Showclix Website
	 * preference
	 *
	 * @return True if preference was saved, false if not
	 */
	public static boolean getSaveShowclix() {
		return !(!myPrefs.getBoolean(PREFTYPES.PAXCHECK_CHECK_SHOWCLIX.name(),
				false) && myPrefs.getBoolean(
				PREFTYPES.PAXCHECK_CHECK_SHOWCLIX.name(), true));
	}
	...
	public static boolean getSavePax() {
		return !(!myPrefs
				.getBoolean(PREFTYPES.PAXCHECK_CHECK_PAX.name(), false) && myPrefs
				.getBoolean(PREFTYPES.PAXCHECK_CHECK_PAX.name(), true));
	}
	...
	public static boolean getSaveAlarm() {
		return !(!myPrefs.getBoolean(PREFTYPES.PAXCHECK_PLAY_ALARM.name(),
				false) && myPrefs.getBoolean(
				PREFTYPES.PAXCHECK_PLAY_ALARM.name(), true));
	}
	...
	public static boolean getSaveEvent() {
		return !myPrefs.get(PREFTYPES.PAXCHECK_EVENT.name(), "NOPE").equals(
				"NOPE");
	}
	...
	public static boolean getSaveEmail() {
		return !myPrefs.get(PREFTYPES.PAXCHECK_EMAIL.name(), "NOPE").equals(
				"NOPE");
	}
	...
	public static boolean getSaveCellnum() {
		return !myPrefs.get(PREFTYPES.PAXCHECK_CELLNUM.name(), "NOPE").equals(
				"NOPE");
	}
	...
	/**
	 * Gets the currently saved refresh time of the program, or 15 if preference
	 * was not saved.
	 *
	...
	/**
	 * Gets whether the program should check the PAX Website, or true if
	...
	/**
	 * Gets whether the program should check the Showclix website, or true if
	 * preference was not saved.
	 *
	 * @return True if the program should check the Showclix website, false if
	 *         not
	 */
	...
	/**
	 * Gets whether the program should play the audio alarm, or false if
	 * preference was not saved.
	 *
	...
	/**
	 * Gets the email address last used, or a blank String if preference was not
	 * saved.
	 *
	...
	/**
	 * Gets the cell number last used, or a blank String if preference was not
	 * saved. This does NOT return the carrier extension if the provider saved
	...
	/**
	 * Gets the last Showclix link found for a specific expo. Note that this
	 * returns in the following format: https://www.showclix.com/Event/EVENTID
	 *
	 * @param expo
	 *            The Expo to check for
	 * @return The last Showclix link for the specific expo
	 */
	public static String getLastEvent(String expo) {
		return myPrefs
				.get(PREFTYPES.PAXCHECK_LAST_EVENT.name() + "_"
						+ expo.toUpperCase(), Browser.getShowclixLink());
	}
	...
	/**
	 * Gets the last Showclix link found for the program's current expo. Note
	 * that this returns in the following format:
	...
	/**
	 * Sets whether or not the program should save the preferences. Note that if
	 * this is set to false, ALL of the preferences will simply be set to false.
	 * Further changes to preferences could result in preferences being saved.
	 *
	 * @param save
	 *            True to save preferences, false to not
	 */
