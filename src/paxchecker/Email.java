...
public class Email {
	...
	/**
	 * Initializes the Email class. Note that this should be run before any
	 * other method in the Email class is used.
	 */
	...
	/**
	 * Sets the current email address (username) for the program to use. Note
	 * that if the username does not contain an ending (&#64;site.net) or is
	 * null, &#64;yahoo.com is automatically appended to the end of the
	 * username. If the site ending does not end in &#64;yahoo.com or
	 * &#64;gmail.com, the program parses the SMTP server addess after the
	 * address ending and two colons.<br>
	 * Example: User&#64;site.com::site.smtp.server<br>
	 * This also (optionall) parses the site's SMTP port after the SMTP server
	 * address and a colon.<br>
	...
	public static void setUsername(String username) {
		...
		if (username.length() < 3) {
			...
			if (!username.contains("::")) {
				...
				ErrorHandler
						.printError(
								"ERROR Using Custom Host",
								"Unable to parse the SMTP server from the given address ("
										+ username
										+ ")! Please make sure this was input correctly!",
								null);
				props.put("mail.smtp.user", "@yahoo.com");
				return;
			}
			...
			if (extraInfo == null) {
				ErrorHandler
						.printError(
								...
								"The SMTP server is required for non-Yahoo or non-GMail addresses. Please put ::SMTP.ser.ver:PORT after the email specified.",
								null);
				props.put("mail.smtp.user", "@yahoo.com");
				return;
			}
	...
	/**
	 * Sets the current host used by the JavaMail API. The host is the TLS/SSL
	 * email server to use. Note that this also explicitly trusts said host.
	 * This opens up the program to a potential MItM attack, however it does fix
	 * the SSL certificate errors that have been popping up.
	 *
	 * @param h
	...
	/**
	 * Gets the currently set host. Note that this just returns the set trusted
	 * host in the Properties.
	 *
	...
	/**
	 * Sets the port used by JavaMail. This port is used to access the email
	 * server.
	 *
	 * @param p
	 *            The port to use
	...
	/**
	 * Encrypts the given password and saves the encrypted password in memory
	 * for later use.
	 *
	 * @param pass
	 *            The password to encrypt and save for later
	 * @see #getPassword()
	 */
		...
		try {
			...
			ErrorHandler
					.printError(
							...
							"An error has occurred while attempting to encrypt your password.",
							e);
		}
	}
		...
		try {
			...
			ErrorHandler
					.printError(
							...
							"And error has occurred while attempting to decrypt your password.",
							e);
		}
		return null;
	}
	...
	public static String getCarrierExtension(String carrier) {
		...
		default:
			System.out
					.println("ERROR: Unable to identify carrier. Using default AT&T.");
			return getCarrierExtension("AT&T (MMS)");
		}
	}

	/**
	 * Sets the {@link #props} settings for the the current email address being
	 * used. Call every time the email provider (Yahoo, GMail) changes.
	 */
	...
	public static boolean testEmail() {
		if (sendMessage(
			...
			System.out.println("There was an error sending your text message.");
		}
		return false;
	}

	/**
	 * Checks whether the program should send an email. If the username OR the
	 * email to send to is null (no valid address/number was given), it returns
	...
	public static boolean shouldSendEmail() {
		return getUsername() != null && !getUsername().equals("@yahoo.com")
	 ...
	 * {@link #setUsername(java.lang.String)},
	 * {@link #setPassword(java.lang.String)}, and (
	 * {@link #setCellNum(java.lang.String, java.lang.String)} or
	...
	public static boolean sendMessage(String subject, String msg) {
		if (!shouldSendEmail()) {
			System.out
					.println("Unable to send email: Program not properly configured.");
			return false;
		}
		createSession();
		try {
			...
			if (getAddressList().size() == 1) {
				message.addRecipient(Message.RecipientType.TO,
						new InternetAddress(getAddressList().get(0)
								.getCompleteAddress()));
			} else {
			...
			ErrorHandler.printError("ERROR",
					"The message was unable to be sent.", mex);
			return false;
		} catch (Exception e) {
			e.printStackTrace();
			ErrorHandler
					.printError(
							"ERROR",
							"An unknown error has occurred while attempting to send the message.",
							e);
			return false;
		}// end catch block
	...
	/**
	 * Sends an email in the background with the given information. Note that
	 * this starts a new background Thread and will NOT block until an email is
	 * sent.
	 *
	 * @param title
	 * @param message
	...
	/**
	 * A new implementation of email addresses to email when a new event is
	 * found. The goal of this is to make it easier to read and write separate
	 * email addresses. This should be used as much as possible to avoid having
	 * String conversions and highly-likely IndexOutOfBoundsExceptions and
	 * NullPointerExceptions being thrown. This will also free up the cluttered
	 * code in Email.java and the Setup GUI.
	 */
	public static final class EmailAddress {
		...
		public String getCarrierName() {
			...
			return getProvider(completeAddress.substring(completeAddress
					.indexOf("@")));
		}
		...
		public void setEmailAddress(String address) {
			if (address == null) {
				...
				return;
			} else if (address.length() < 5) { // Seriously if it's less than
												// ~10 characters it's probably
												// invalid, but 4 or less is
												// just absurd.
				System.out.println("ERROR: Email address is too short!");
				return;
			} else if (!address.contains("@")) {
				System.out.println("NOTE: Address " + address
	 ...
	 * that this does NOT check for duplicates.
	 *
	 * @param add
	...
	/**
	 * Adds the given email addresses to the list of addresses to send to. Note
	 * that this does NOT check for duplicates.
	 *
	 * @param add
	 *            The addresses to add to the list
	 */
	...
	public static void removeEmailAddress(String remove) {
		...
		while (myIt.hasNext()) {
			...
			if (next.getCompleteAddress().matches(remove)) {
				...
				System.out.println("Removed email address " + remove
						+ " from the list of addresses.");
				return;
			}
		}
		System.out.println("Could not remove address " + remove
				+ " from list of addresses.");
	}

	/**
	 * Removes the given email address from the list of addresses to send to.
	 * Note that removing
	 *
	 * @param remove
	 ...
	 * that these addresses should be separated with semicolons (";").
	 *
	 * @param addresses
	 *            The addresses to convert to a list
	...
	public static List<EmailAddress> convertToList(String addresses) {
		...
		for (String split1 : split) {
			...
			if (temp.isValid()) {
				...
				System.out.println("ERROR: Temp address " + split1
						+ " is not valid!");
			}
		}
		return tempList;
	 ...
	 * addresses. Note that each address will be separated by semicolons (";").
	 *
	 * @param list
	 *            The List to convert
	...
	/**
	 * Gets the current List of addresses that the program will send texts to.
	 * This List is final and cannot be reassigned.
	 *
